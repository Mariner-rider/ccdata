...
class QuestionCrawler:
    ...
    async def crawl_source(self, source: dict | str, exam_type: str) -> list[Question]:
        """
        Crawls the source URL for the given exam type and extracts MCQs.
        """
        self._current_exam_type = exam_type
        source_config = source if isinstance(source, dict) else {"site": self._site_from_url(source), "base_url": source}
        ...
        for page_url, html in html_pages:
            questions.extend(self._extract_questions(html, page_url, source_config["site"]))
        return questions
    ...
    def _site_from_url(url: str) -> str:
        return __import__("urllib.parse", fromlist=["urlparse"]).urlparse(url).netloc.replace("www.", "").split(".")[0] or "unknown"
