...
def extract_fallback(url: str, timeout: float = 20.0) -> dict:
    ...
    location=_pick_line(lines,["location","address","campus"])
    courses=[l for l in lines if any(k in l.lower() for k in ["b.tech","mba","course","program"])]
    fees=list({*re.findall(r"\$\s?\d[\d,]*", text), *re.findall(r"\b(?:INR|Rs\.?|USD)\s?\d[\d,]*", text, re.I)})
    admission=[h for h,t in p.links if "admission" in h.lower() or "apply" in h.lower() or "admission" in t.lower()]
    placement=[l for l in lines if "placement" in l.lower()]
    faculty=[l for l in lines if "faculty" in l.lower() or "prof" in l.lower() or "dr." in l.lower()]
    hostel=[l for l in lines if "hostel" in l.lower()]
