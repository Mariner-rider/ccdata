...
def _search(query, entity_type=None, location=None):
    ...
    with sqlite3.connect(Repo(_cfg().database_url).path) as c: rows=c.execute('select id,entity_type,title,payload from crawler_records').fetchall()
    for rid,et,title,p in rows:
        rec=json.loads(p or '{}'); f=rec.get('fields',{})
        text=' '.join([str(title or ''),' '.join(f.get('courses',[]) if isinstance(f.get('courses'),list) else []),str(f.get('location') or ''),str(rec.get('info',{}).get('name') or '')]).lower()
        if q and q not in text: continue
        if entity_type and et!=entity_type: continue
        if location and location.lower() not in str(f.get('location','')).lower(): continue
        out.append({'id':rid,'entity_type':et,'title':title,'location':f.get('location',''),'score':text.count(q) if q else 1})
